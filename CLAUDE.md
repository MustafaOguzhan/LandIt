...
- Monetization: **7-day free trial, card required to start**, then a
  **paid subscription — $9/month or $49/year** (owner's decision, cut down
  from $18/$89 — the prior price, itself revised down from an initial
  $24/$99, then back up from $13/month once the AI content-writing feature
  — see status table below — narrowed the gap with Rezi.ai, ~$29/mo).
  Yearly at $49 is still a real discount vs. paying monthly all year
  (~55% off, vs. ~59% when monthly was $18 — the percentage moves with the
  monthly price). No lifetime/one-time-payment tier — explicitly ruled out
  by the owner due to the unbounded long-term support liability of a
  one-time payment.
...
| Job search listings | **Real, via two providers, 61 countries total.** `api/jobs.js` proxies to Adzuna (19 countries) or Careerjet (42 more, spanning Europe/Middle East/Asia-Pacific/Latin America — added specifically to close the Norway gap, since Adzuna has no coverage there at all, then expanded to nearly all of Careerjet's remaining locale list once the integration was proven working) depending on which country is selected, keeping both providers' credentials secret server-side; results replace the old 3 static cards. Match % is computed client-side, word-level, against the resume — deliberately not phrase-level like the Keyword Targeting section, since real job-posting prose rarely repeats resume wording verbatim and phrase matching under-scored obvious fits during testing. Needs an Adzuna account (free) and, for the extra 42 countries, a Careerjet publisher account (see README for both) — Careerjet's is a free affiliate/publisher signup, not a paid API, and requires site info + email activation. Russia is deliberately excluded (sanctions/compliance judgment call, flagged for the owner rather than silently included) — the remaining gap vs. Careerjet's 70+ locales is mostly duplicate language variants of countries Adzuna already covers. More can be added by extending `CAREERJET_LOCALES` in `api/jobs.js` and the country dropdown in `landit.html`. |
...
| Payments / subscription (7-day trial → $9/mo or $49/yr) | **Real, via Stripe.** The 7-day trial requires a card up front (Stripe Checkout with `trial_period_days: 7`) and auto-converts to a real charge when it ends unless canceled first — see "Trial auto-converts to paid" in Business model above. `api/create-checkout-session.js` takes a `plan` ('monthly'/'yearly'), maps it to the matching Stripe Price ID, and creates a real Checkout Session for a first-time subscriber; for someone who already has a live (active/trialing) subscription, switching plans updates that same subscription's price in place instead of starting a second one (fixed after live-mode testing surfaced a double-subscription risk). `api/stripe-webhook.js` verifies Stripe's webhook signature and updates `profiles.subscription_status` (trialing/active/past_due/canceled) so the nav trial/Pro badge reflects reality — the frontend also polls briefly right after a Checkout redirect since the webhook can land a few seconds after the browser does. Needs a Stripe account + product with both prices + webhook configured (see README). **Feature-gating is real**: the resume builder and ATS score/keyword targeting stay free for everyone (no login needed), but PDF download, the AI mock interview, and job search each check `hasActiveAccess()` (logged in + trialing-with-days-left or subscription_status === 'active') before running — logged-out users get the signup modal, logged-in users past their trial get a message and a scroll to Pricing. `past_due`/`canceled` are treated as no access. Deleting an account cancels any live subscription first (trialing included), so a deleted account can't still get charged later. |
...
| Transactional auth emails (signup confirmation, password reset, etc.) | **Real, via Supabase custom SMTP + Resend.** Supabase's own email sending is rate-limited and meant for testing only — configured (dashboard-only, no code) under Authentication → Emails → SMTP Settings to route through Resend instead: host `smtp.resend.com`, sender `hello@mail.landitai.com` on the same Resend account/domain already used for churn-feedback emails, scoped to a dedicated API key restricted to that one domain. "Confirm email" (Authentication → Sign In / Providers) is back on now that delivery is reliable. Verified end-to-end with a real signup — confirmation email arrived correctly from `hello@mail.landitai.com`. |
| Free ATS Checker landing page (`ats-checker.html`) | **Real, standalone, no login required.** A dedicated SEO-targeted page (own title/meta/OG image/canonical, in `sitemap.xml`, linked from the main site's footer) built to capture "free ats resume checker" search demand — the same category of query Jobscan built its entire business on. Same live-scoring engine as the main builder's ATS Score section (content/impact/keywords/formatting, animated score ring, quick-win suggestions, keyword match chips) against a simplified structured form (no photo/template/PDF export — this page is score-only, English-only, and doesn't save anything, by design, to keep it a fast, frictionless lead magnet). **Also supports uploading a PDF or DOCX résumé to auto-fill the form** (pdf.js / mammoth.js, both loaded from cdnjs, parsing entirely client-side — the file itself is never uploaded anywhere): a heuristic pass looks for common section headers (Summary/Experience/Education/Skills) and, within Experience, lines containing a year or "Present" to split entries. Real résumés vary too much for this to be perfect, so the result always lands in the editable fields with a visible "please double-check" note rather than being treated as final — manual entry with no upload remains the default path. **The scoring logic is a deliberate duplicate of `landit.html`'s copy, not a shared module** — extracting a shared engine would mean touching the main product's live, revenue-critical scoring path for a marketing page's sake; each file has a comment cross-referencing the other so a future scoring change is a reminder to update both. Ends with a CTA back to the main site's trial signup. |
| Free Resume Templates landing page (`templates.html`) | **Real, standalone, no login required.** Targets "free resume templates" search demand — a very high-volume query in this space. Shows all 4 real templates (Clearline/Fieldwork/Groundwork/Runway) side by side, each rendering the *exact same* example résumé through the actual production markup/CSS copied verbatim from `landit.html`'s live-preview classes (`.live-resume`, `.lname`, `.lexp-block`, the per-template `[data-template]` overrides, etc.) — not a mockup, so the comparison is honest and can't visually drift from what the builder actually produces. Each template's "Use this template" button links to `landitai.com/?template=<name>#builder`; `landit.html` reads that `?template=` query param on load, calls the existing `selectTemplate()`, scrolls to the builder, and strips the param from the URL — so a visitor lands with that exact template already applied instead of always seeing the Clearline default. Own title/meta/OG image/canonical, in `sitemap.xml`, linked from the main site's footer (all 8 languages). |
| Multi-language support (EN/TR/DE/NO/ES/FR/PT/IT) | **Real.** A `data-i18n`-attribute + dictionary system in `landit.html` covers essentially all site chrome — marketing copy, the resume builder form, ATS score/keyword targeting/job search sections, pricing, the auth/account modals, and dynamically-generated text (score "quick win" tips, validation errors, toasts). Language auto-detects from the visitor's browser on first visit, is always overridable via the nav dropdown, and the choice is remembered (`localStorage`) — no page reload needed, everything re-renders in place. The AI mock interview and the "Improve with AI" writer both received a `lang` parameter and genuinely reply/write in the selected language server-side (`api/interview.js`, `api/generate-content.js`), not just the surrounding UI. **Deliberately left in English regardless of site language**, as reasonable scope lines: the resume builder's own month/year date-picker options (their abbreviations are parsed back out of saved/example resumes, so translating them risked real round-trip bugs for a cosmetic win); the 4 template names (Clearline/Fieldwork/Groundwork/Runway — product names, like "LandIt" itself); the 61-country job-search dropdown; the built-in example-resume content (profession bullet points shown via "See it filled in"); and `privacy.html`/`terms.html`, which don't yet have the switcher. More languages are a matter of extending the same dictionary, not new architecture. |
